...
if [ "$machine" = "hpc" ]
then 
echo "You will now need to edit the user.mak file to make your copy of cGENIE speak to your netcdf libraries"
sleep 1s
echo "First, open cgenie.muffin/genie-main/user.mak in your chosen text editor..."
echo "Then, copy out line 77 (with a #)."
echo "Then, add two lines after line 91."
echo "Add the text (on new lines 92 and 93)..."
echo "### Iridis/Lyceum (should work for Red Hat HPC facilities generally) ###"
echo "NETCDF_DIR=$(HOME)"
sleep 1s
echo "Note that you could get away with just adding the text on new line 93 - but this would be messy and leave you with poorly documented code."
read -p "Once you've edited user.mak, press any key! Note that cGENIE will *not* run on Iridis (/Red Hat HPC accounts more broadly) unless you do this..." -n1 -s
fi
fi
